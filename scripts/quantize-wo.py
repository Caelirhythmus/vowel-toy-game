#!/usr/bin/env python3
"""weight-only QDQ 量化：Conv/MatMul/ConvTranspose 权重 int8 对称量化。

图里只新增 DequantizeLinear 节点（onnxruntime-web wasm 确定支持；
onnxruntime 自带的 quantize_dynamic 会融合出 ConvInteger——wasm 无实现）。
...
def main() -> None:
    ...
    onnx.save(model, dst)
    print(f'[quantize-wo] {len(quantized)} 个权重 int8 量化完成: {dst}')
